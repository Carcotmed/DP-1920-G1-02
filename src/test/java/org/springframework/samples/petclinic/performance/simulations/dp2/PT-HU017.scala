...
class PTHU017 extends Simulation {
...
object Home {
	...
	object EditOrderPos{
		val editOrderPos = exec(http("EditOrderPos")
			.get("/orders/edit/1")
			.headers(headers_0)
			.check(css("input[name=_csrf]","value").saveAs("stoken"))
		).pause(14)
	...
	object EditOrderNeg{
		val editOrderNeg = exec(http("EditOrderNeg")
			.get("/orders/edit/1")
			.headers(headers_0)
			.check(css("input[name=_csrf]","value").saveAs("stoken"))
		).pause(14)
...
setUp(
		editOrderPositive.inject(rampUsers(200) during (10 seconds)),
		editOrderNegative.inject(rampUsers(200) during (10 seconds))
